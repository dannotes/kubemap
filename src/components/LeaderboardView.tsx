...
export function LeaderboardView() {
  ...
  const data = useMemo(() => {
    ...
    // Dual title holders: Golden Kubestronaut + CNCF Ambassador
    const dualTitle = people
      .filter(p => p.tier === 'golden' && p.isAmbassador)
      .sort((a, b) => a.name.localeCompare(b.name))
      .slice(0, 10);

    return { topCountries, topCompanies, topCities, goldDensity, topContributors, dualTitle };
  }, [people]);
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 10 }}>
          <h2 style={{ fontFamily: "'DM Sans', sans-serif", fontWeight: 700, fontSize: 17, letterSpacing: '-0.02em', color: 'var(--text)' }}>Leaderboard</h2>
          <span style={{ fontSize: 11, color: 'var(--text-muted)', fontFamily: "'JetBrains Mono', monospace", letterSpacing: '0.04em', textTransform: 'uppercase' }}>community rankings</span>
        </div>
      </div>
      <div style={{ flex: 1, overflow: 'auto' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: 16, padding: '18px 22px' }}>
          {/* Top countries */}
          <LbCard title="Top countries" meta={`${data.topCountries.length} on the map`}>
          ...
          {/* Top companies */}
          <LbCard title="Top companies" meta="by Kubestronaut headcount">
          ...
          {/* Top cities */}
          <LbCard title="Top cities" meta="hottest Kubestronaut hubs">
          ...
          {/* Golden density */}
          <LbCardCompact title="Golden density" meta="% Golden per country">
          ...
          {/* Top CNCF project contributors */}
          <LbCardCompact title="Top contributors" meta="most CNCF projects">
          ...
          {/* Dual title holders */}
          {data.dualTitle.length > 0 && (
            <LbCardCompact title="★ Dual title" meta="Golden + Ambassador">
              {data.dualTitle.map((p, i) => (
                <div key={p.id} onClick={() => openProfile(p.id)} style={{
                  display: 'grid', gridTemplateColumns: '28px 32px 1fr auto',
                  alignItems: 'center', gap: 10, padding: '7px 12px', borderRadius: 8, cursor: 'pointer',
                }}
                  ...
                  onMouseLeave={e => (e.currentTarget.style.background = '')}
                >
                  <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 11, color: i < 3 ? 'var(--gold-soft)' : 'var(--text-muted)', fontWeight: i < 3 ? 800 : 600, textAlign: 'right' }}>{i + 1}</div>
                  <div style={{ width: 28, height: 28, borderRadius: 8, overflow: 'hidden', background: avColor(p.name), display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10.5, fontWeight: 700, color: '#0b1220', position: 'relative' }}>
                    {p.image && <img src={imageUrl(p.image)!} loading="lazy" onError={e => (e.currentTarget.style.display = 'none')} style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover' }} />}
                    <span>{initials(p.name)}</span>
                  </div>
                  <div style={{ minWidth: 0 }}>
                    <div style={{ fontFamily: "'DM Sans', sans-serif", fontWeight: 600, fontSize: 13, color: 'var(--text)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{p.name}</div>
                    <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10.5, color: 'var(--text-muted)' }}>{p.flag} {p.country}{p.company ? ` · ${p.company}` : ''}</div>
                  </div>
                  <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
                    <span style={{ fontFamily: "'DM Sans', sans-serif", fontWeight: 700, fontSize: 13, color: 'var(--gold-soft)' }}>★</span>
                    <span style={{ fontSize: 8.5, fontWeight: 700, background: 'rgba(168,85,247,0.2)', color: '#c084fc', borderRadius: 3, padding: '1px 4px' }}>A</span>
                  </div>
                </div>
              ))}
            </LbCardCompact>
...
function LbCard({ title, meta, children }: { title: string; meta: string; children: React.ReactNode }) {
  return (
...
function LbCardCompact({ title, meta, children }: { title: string; meta: string; children: React.ReactNode }) {
  return (
    <div style={{ background: 'var(--panel-solid)', border: '1px solid var(--border)', borderRadius: 12, overflow: 'hidden' }}>
      <div style={{ padding: '14px 16px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: 8 }}>
        <h3 style={{ fontFamily: "'DM Sans', sans-serif", fontWeight: 700, fontSize: 14, color: 'var(--text)', letterSpacing: '-0.01em', whiteSpace: 'nowrap' }}>{title}</h3>
        <span style={{ fontSize: 10, color: 'var(--text-muted)', fontFamily: "'JetBrains Mono', monospace", textTransform: 'uppercase', letterSpacing: '0.06em', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{meta}</span>
      </div>
      <div style={{ padding: '8px 4px' }}>{children}</div>
    </div>
  );
